...
public class PhysicsThread extends KinetixThread
{
    ...
    private void initializeState(SimulationState state)
    {
        ...
        state.settings.width = 600;
        state.settings.height = 600;
        ...
        double temperature = Kinetix.oneAndOnly.temperature;
        for (int velocity = 0; velocity < MAXIMUM_VELOCITY; velocity++)
        {
        	...
        	{
        		double mass = (type == Atom.ATOM_RED) ? Kinetix.oneAndOnly.mass1 : Kinetix.oneAndOnly.mass2;

        		final double BOLTZMANN = 1.38064852;
        		final double DALTON = 1.660539040;
        		
        		double a = Math.pow((mass * DALTON * 0.0001) / (2 * Math.PI * BOLTZMANN * Kinetix.oneAndOnly.temperature), 1.5);
        ...
        for (int i = 0; i < 200; i++)
        {
        	int type = (i < 100) ? Atom.ATOM_RED : Atom.ATOM_GREEN;
        	double radius = (type == Atom.ATOM_RED) ? Kinetix.oneAndOnly.radius1 : Kinetix.oneAndOnly.radius2;
        	double mass = (type == Atom.ATOM_RED) ? Kinetix.oneAndOnly.mass1 : Kinetix.oneAndOnly.mass2;
